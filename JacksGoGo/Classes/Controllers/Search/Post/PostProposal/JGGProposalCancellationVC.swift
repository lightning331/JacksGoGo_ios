//
//  JGGProposalCancellationVC.swift
//  JacksGoGo
//
...
class JGGProposalCancellationVC: JGGPostAppointmentBaseTableVC, UITextFieldDelegate {
    ...
    private func showOriginalProposal() {
        ...
        if let allowed = proposal?.cancellationAllowed {
            if allowed {
                btnAllow.select(true)
                let totalTime = proposal?.cancellationTime ?? 0
        ...
        txtTerms.text = proposal?.cancellationNote
    }
    ...
    override func onPressedNext(_ sender: UIButton) {
        if btnAllow.selected() {
            ...
            if totalSeconds == 0 {
                Toast(text: LocalizedString("Please enter cancellation time."), delay: 0, duration: 3).show()
                return
            }
        }
    ...
    override func updateData(_ sender: Any) {
        if let parentVC = parent as? JGGProposalStepRootVC {
            if btnAllow.selected() {
                ...
                let minutes = Int(txtMinutes.text ?? "0") ?? 0
                let totalSeconds = (days * 3600 * 24) + (hours * 3600) + (minutes * 60)
                parentVC.proposal?.cancellationTime = Double(totalSeconds)
                parentVC.proposal?.cancellationAllowed = true
                parentVC.proposal?.cancellationNote = txtTerms.text
            } else {
                if btnNoAllow.selected() {
                    parentVC.proposal?.cancellationAllowed = false
                } else  {
                    parentVC.proposal?.cancellationAllowed = nil
                }
                parentVC.proposal?.cancellationNote = nil
                parentVC.proposal?.cancellationTime = nil
            }
        }
    }
